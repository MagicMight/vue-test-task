<template>
	<div class="editor-container" v-if="note"  ref="editor-container">
		...
		<div class="actions-panel" ref="actions-panel">
			<a  @click="doAction('undo')" 
				class="action__undo" 
				title="Отменить изменение"  />
			
			<a 	@click="doAction('redo')" 
				class="action__redo" 
				title="Повторить изменение" />

			<a 	@click="doAction('save')" 
				class="action__save" 
...
export default {
	...
	methods: {
		...
		doAction( action ) {
			switch(action) {
				...
				case 'register':
					...
					this.$set(this.changeStack, this.changeStack.length, JSON.parse(JSON.stringify(this.note)))
				...
				case 'save':
					this.changeStack = this.changeStack.filter( (el, idx) => idx === this.changeIndex);
					this.changeIndex = 0;
					this.note = this.changeStack[this.changeIndex];
				...
				case 'dropLastState':
					if( this.changeIndex !== 0 && this.changeIndex !== this.changeStack.length - 1 ) {
		...
		createTask() {
			this.doAction('register');
			this.note = this.changeStack[ this.changeIndex ];
			let id = this.note.tasks.length > 0 ? this.note.tasks[ this.note.tasks.length-1 ].id + 1 : 1;
			this.note.tasks.push({id, title: 'Новая задача #'+id, isComplete: false});
		}
	},
