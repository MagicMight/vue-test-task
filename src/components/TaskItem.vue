<template>
	<div class="taskitem" :class="{completed: isComplete}">
		<button 
			v-if="!preview"
			@click="$emit('remove-task', task_id)"
			><i class="remove-task-img"></i></button>

		<span
			:title="task.title"
			v-if="!editMode" 
			@click="switchMode">{{ task.title }}</span>
...
export default {
	...
	computed: {
		isComplete: {
			get: function() {  return this.task.isComplete  },
			set: function() {  this.task.isComplete = !this.task.isComplete  }
		}
	}
}
</script>
...
.preview .taskitem.completed {
	...
	background-size: 1.3em 1.3em, auto .9em;
	background-repeat: no-repeat, repeat-x;
